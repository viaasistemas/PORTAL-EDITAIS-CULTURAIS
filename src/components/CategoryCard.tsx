...
const CategoryCard = ({ imageIcon, title, description, center = false }: CategoryCardProps) => {
  return (
    <div className={`bg-white p-8 rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-all duration-300 flex flex-col h-full group ${center ? 'text-center items-center' : 'text-left'}`}>
      <div className={`flex mb-6 ${center ? 'flex-col items-center gap-3' : 'items-center gap-4'}`}>
      ...
      <p className={`text-slate-500 text-sm leading-relaxed font-medium ${center ? 'text-center' : ''}`}>
        {description}
      </p>
    </div>
  );
